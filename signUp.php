...
if (isset($_SESSION['name'])) {
  echo 'ようこそ' . htmlspecialchars($_SESSION['name'], ENT_QUOTES, 'utf-8'). "さん<br>";
...
<?php if ( isset($errorss) ):  ?>
  <?php foreach ($errorss as $error):?>
     <p><?=htmlspecialchars( $error, ENT_QUOTES, 'utf-8' )//入力エラーの表示?></p>
  <?php endforeach;?>
<?php endif ?>
     ...
     <input type="submit" name="send" value="Sign in!" class="button">
   </form>
   ...
   <?php endif ?>
    ...
    <br><input type="text" name="username"></br>
     ...
     <input type="submit" name="send" value="Sign Up!" class="button">
